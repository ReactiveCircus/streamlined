...
class Navigator(private val fragment: Fragment) {
    ...
    ) = navControllerByType(navControllerType).safeNavigate(destination, createBundle(args))

    fun <Input : Parcelable> requireNavInput(): Input =
        fragment.requireArguments().getParcelable<Input>(NAV_ARGS_KEY)!!

    fun navControllerByType(
        navControllerType: NavControllerType = NavControllerType.Parent
    ): NavController = when (navControllerType) {
        NavControllerType.Root -> fragment.requireActivity()
            .findNavController(R.id.rootNavHostFragment)
        NavControllerType.Parent -> fragment.findNavController()
    ...
    companion object {
        ...
        fun createBundle(args: Parcelable?): Bundle? = args?.let {
